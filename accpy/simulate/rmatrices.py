...
def dipole(ev, y):
    ...
    [0.,            0.,                 1., L,  0., 0],
    [0.,            0.,                 0., 1., 0., 0],
    [-sinalpha,     -rho*(1.-cosalpha), 0., 0., 1., rho*(alpha/(y**2)-alpha+sinalpha)],
    [0.,            0.,                 0., 0., 0., 1]
    ])
    return R
...
def rxdmatrix(elementvec):
    ...
    typ = elementvec[0]     # type of element (dipole, quadrupole, ...)
    if typ == 0:
        rmatrix = xd_drift(elementvec)
        return rmatrix
    elif typ == 1:
        rmatrix = xd_dipole(elementvec)
        return rmatrix
    elif typ == 2:
        rmatrix = xd_edge(elementvec)
        return rmatrix
    elif (q == 0 and typ == 3) or (q == 0 and typ == 4):
        rmatrix = xd_drift(elementvec)
        return rmatrix
    elif typ == 3:
        rmatrix = xd_quadrf(elementvec)
        return rmatrix
    elif typ == 4:
        rmatrix = xd_quadaf(elementvec)
        return rmatrix
    elif typ == 5:
        rmatrix = xd_rotator(elementvec)
        return rmatrix
    elif typ == 6:
        rmatrix = xd_solenoid(elementvec)
        return rmatrix
